...
public class TrafficSimulationGraph {
    ...
    public TrafficSimulationGraph(List<Road> roadRecords, List<Cross> crossRecords) {
        ...
        for (Road roadRecord : roadRecords) {
            ...
            if (roadRecord.isDuplex()) {
                DirectedRoadId reverseDirectionRoadId = new DirectedRoadId(roadRecord.getId(), true);
                roads.put(reverseDirectionRoadId, new SimulationRoad(reverseDirectionRoadId, roadRecord.getLength(), roadRecord.getSpeed(), roadRecord.getChannel()));
            }
        }
    ...
    private boolean willGoThroughCross(SimulationRoadCar p1wCar, SimulationRoad road, SimulationRoad roadOut) {
    ...
    FullSimulationResult simulateOneTimePeriod(SimulationStats simulationStats) {
        final boolean failOnCantStart = simulationStats.failOnCantStart;
        final LinkedList<SimulationGarageCar> garageCarsSortedByStartTimeAndId = simulationStats.garageCarsSortedByStartTimeAndId;
        final HashMap<Integer, SimulationRoadCar> roadCars = simulationStats.roadCars;
        final List<CarSimulationResult> carSimulationResults = simulationStats.carSimulationResults;
        final int time = simulationStats.time;
                ...
                do {
                    ...
                    for (SimulationRoadWithCrossDirection roadAndDirection : cross.roadsInSortedById) {
                        ...
                        do {
                            ...
                            if (/*TODO*/!p1wCar.isOnLastRoad() && willGoThroughCross(p1wCar, road, cross.roadsOut[directionOut]) &&
                                    turn.getAllWithHigherPriority().stream().anyMatch(higherPriorityTurn -> {
                            ...
                            int speed = min(road.speed, p1wCar.speed);
                            boolean skipFirstInChannel;
                            if (p1wCar.isOnLastRoad()) {
                                int maxNewPosition = p1wCar.getPosition() + speed;
                                if (maxNewPosition >= road.length) {
                                    ...
                                    simulationStats.totalTravelTime += time - p1wCar.planTime;
                                    ...
                                    throw new AssertionError();
                                        /*// Identical to below when s1P1 > v1
                                        p1wCar.scheduleToPosition(maxNewPosition);
                            ...
                            while (channelCars.hasNext()) {
                                ...
                                if (frontCar == null) {
                                    ...
                                    //if (frontCar.isWaiting()) throw new AssertionError();
        ...
        while (garageCarIterator.hasNext()) {
            ...
            if (scheduleToNewRoad(car, road, speed - 1) == ScheduleToNewRoadResult.SUCCESS) {
                ...
                clearSimulation();
                return FullSimulationResult.newCantStartOnTimeInstance(time, simulationStats.totalTravelTime, carSimulationResults);
            } else
                simulationStats.cantStartOnTime = true;
        }
    ...
    static class SimulationStats {
        boolean failOnCantStart;
        LinkedList<SimulationGarageCar> garageCarsSortedByStartTimeAndId;
        HashMap<Integer, SimulationRoadCar> roadCars;
        List<CarSimulationResult> carSimulationResults;
        int time;
        boolean cantStartOnTime;
        int totalTravelTime;

        SimulationStats(boolean failOnCantStart, LinkedList<SimulationGarageCar> garageCarsSortedByStartTimeAndId, HashMap<Integer, SimulationRoadCar> roadCars, List<CarSimulationResult> carSimulationResults, int time, boolean cantStartOnTime, int totalTravelTime) {
            this.failOnCantStart = failOnCantStart;
            this.garageCarsSortedByStartTimeAndId = garageCarsSortedByStartTimeAndId;
            this.roadCars = roadCars;
            this.carSimulationResults = carSimulationResults;
            this.time = time;
            this.cantStartOnTime = cantStartOnTime;
            this.totalTravelTime = totalTravelTime;
        }
    }

    // Simulate in the condition that a car starts as early as possible if it can't start at start time
    FullSimulationResult simulate(List<CarStartTimeTurnPathSingleSolution> singleSolutions, boolean failOnCantStart) {
        ...
        SimulationGarageCar firstGarageCar = garageCarsSortedByStartTimeAndId.peekFirst();
        if (firstGarageCar == null)
            return FullSimulationResult.newSuccessInstance(0, 0, Collections.emptyList());
        int minPlanTime = firstGarageCar.startTime;
        ...
        SimulationStats simulationStats = new SimulationStats(failOnCantStart, garageCarsSortedByStartTimeAndId, roadCars, carSimulationResults, -1, false, 0);
        for (simulationStats.time = minPlanTime; !(garageCarsSortedByStartTimeAndId.isEmpty() && roadCars.isEmpty()); simulationStats.time++) {
            FullSimulationResult fullSimulationResult = simulateOneTimePeriod(simulationStats);
            if (fullSimulationResult != null)
                return fullSimulationResult;
        }

        return FullSimulationResult.newSuccessInstance(simulationStats.cantStartOnTime, simulationStats.time - 1, simulationStats.totalTravelTime, carSimulationResults);
    }
    ...
    public List<TurnPath> convertPathsToTurnPaths(List<IntObjPair<Path>> fromPathPairs) {
